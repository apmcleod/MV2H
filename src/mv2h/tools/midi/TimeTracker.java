...
public class TimeTracker {
    ...
    public void addTimeSignatureChange(MidiEvent event, MetaMessage mm) {
		 ...
		 * SPECIAL CASE
		 * ------------
		 * Musescore3 outputs a single 1/1 time signature at the beginning of a MIDI file if the
		 * score had no time signature.
		 *
		 * This assumes any 1/1 time signature at time 0 is from that, and defaults instead to 4/4,
		 * like the MIDI standard.
		 ***************/
		if (event.getTick() == 0 && ts.getNumerator() == 1 && ts.getDenominator() == 1) {
